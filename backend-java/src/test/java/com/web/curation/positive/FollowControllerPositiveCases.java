...
public class FollowControllerPositiveCases {
	...
	Logger logger = LoggerFactory.getLogger(FollowControllerPositiveCases.class);
	...
	@Test
	@Order(2)
	void followAddTest() throws Exception {
	...
	@Test
	@Order(3)
	void followAddTrySecondTest() throws Exception {
		mockMvc.perform(MockMvcRequestBuilders.post("/follow")
			.header("accessToken", accessToken)
			.contentType("application/json")
			.content(new JSONObject(oMapper.writeValueAsString(input)).toString()))
	...
	@Test
	@Order(4)
	void followerListGetTest() throws Exception {
	...
	@Test
	@Order(5)
	void followingListGetTest() throws Exception {
		mockMvc.perform(MockMvcRequestBuilders.get(
			"/follow/following/targetId="+input.get("fromUserId")+"&&loginId="+input.get("fromUserId")))
			.andExpect(MockMvcResultMatchers.status().is2xxSuccessful())
			.andDo(MockMvcResultHandlers.print());
	}
	...
	@Test
	@Order(6)
	void followDeleteTest() throws Exception {
